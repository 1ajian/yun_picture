...
public abstract class PictureUploadTemplate {
    ...
    private UploadPictureResult getUploadPictureResult(String uploadFileName, String uploadPath, File file) {
        ...
        //设置图片的分类(AI)生成
        DetectLabelResponse detectLabelResponse = cosManager.getCategoriesRequest(originalUrl);
        DetectLabelItem[] labels = detectLabelResponse.getLabels();
        //获取分类并设置返回
        List<String> categories = Arrays.stream(labels)
                .filter(label -> label.getConfidence() > 0.7)
                .map(DetectLabelItem::getName).collect(Collectors.toList());
        uploadPictureResult.setCategories(categories);
