package com.jianzhao.picturebackend.manager;
...
public class CosManager {
     ...
     * @return
     */
    public DetectLabelResponse getCategoriesRequest(String url) {
        try {
            //创建凭证
            Credential cred = new Credential(cosClientConfig.getSecretId(), cosClientConfig.getSecretKey());

            //创建客户端
            TiiaClient client = new TiiaClient(cred, "ap-guangzhou");
            // 创建请求对象
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "获取图片分类服务异常:" + e.getMessage());
        }
    }
    ...
    public PutObjectResult putPictureObject(String key,File file) {
        ...
        if (file.length() > 2 * 1024) {
            ...
            thumbnailRule.setRule(String.format("imageMogr2/thumbnail/%sx%s>", 800,800));
...
//            result = new ByteArrayOutputStream();
        try (COSObject object = cosClient.getObject(getObj);
             COSObjectInputStream objectContent = object.getObjectContent();
